...
function spawnStar(pos, size) {
    let obj = new GameObject(
        random() > 0.01 ? "Assets/star.png" : "Assets/patrick.png",
        pos,
        vec2.fromValues(size, size),
        "plant-deco")
    obj.setOrientation(random() * 360)
    level.addObject(obj);
}
...
export function generateRopePath(map_data) {
    ...
    while (prev[current]) {
        ...
        for (let i = 0; i < N; i++) {
            ...
            vec2.add(randomOffset, randomOffset, vec2.fromValues(random() - 0.5, random() -  0.5));
    ...
    {
        let randOffset = vec2.fromValues(random() - 0.5, random() -  0.5)
...
function generateDecoForNode(segment_pos) {
    let n = random() * MAX_DECO_PER_UNIT;

    for (let i = 0; i < n; i++) {
        let offset_vec = vec2.random(vec2.create());
        let size = random() * 0.2 + 0.3;
            ...
            switch(type) {
                ...
                case 1:       //corner
                    if (tl == 1) {
                        ...
                        if (spawn_deco) {
                            let m = random() * MAX_ALGAE_PER_UNIT;
                            for (let i = 0; i < m; i++) {
                                let size = random() * 0.4 + 0.8;
                                let x = random() * 0.5;
                ...
                case 2:       //wall or corridor
                    if ((tl == br) && (tr == bl)) { //corridor
                        ...
                        if (tl == 1 && tr == 1) {
                            ...
                            shape = COLLISION_SHAPES.hor;
                            if (spawn_deco) {
                                let m = random() * 1.5 * MAX_ALGAE_PER_UNIT;
                                for (let i = 0; i < m; i++) {
                                    let size = random() * 0.4 + 0.8;
                                    spawnAlgaeAt(vec2.fromValues(w * GRID_SIZE - side_offset + (random() - 0.5) * GRID_SIZE, -(h * GRID_SIZE + depth_offset + 0.1) + size / 2), size);
                                }
                                if (random() < CORAL_PROBABILITY)
                                {
                                    let size = random() * 0.4 + 0.8;
                                    let x = random() * 0.5;
                                    spawnCoralAt(vec2.fromValues(w * GRID_SIZE - side_offset + (random() - 0.5) * GRID_SIZE, -(h * GRID_SIZE + depth_offset + 0.1) + size / 2), size);
                                }
                            }
                        } else if (bl == 1 && tl == 1) {
                            transform = Transformation.BOTTOM_LEFT;
